...
const EventDashboard = ({ darkMode }) => {
  ...
  const handleApproveAttendee = async (attendeeId) => {
    try {
      ...
      setAttendees(prev => prev.map(a => (a._id || a.id) === attendeeId ? { ...a, isVerified: true } : a));
    } catch (error) {
                ...
                {filteredAttendees.length === 0 ? (
                  ...
                  <td className="px-6 py-4">{att?.type || att?.ticketType || 'Standard'}</td>
                    ...
                    {att?.isVerified === false && (
                      <button onClick={() => handleApproveAttendee(att?._id || att?.id)} className="text-xs text-emerald-600 hover:text-emerald-700 font-bold hover:underline border border-emerald-500/30 px-3 py-1.5 rounded-full bg-emerald-500/10">
                            Approve
                          </button>
                        )}
                      </td>
            ...
            >
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4 text-left">
                ...
                <div className="pt-4 flex gap-3">
                  ...
                  <button type="button" onClick={handleDelete} disabled={isSaving} className={`px-6 py-3.5 rounded-2xl font-bold transition-colors bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20 disabled:opacity-50`}>
                    Delete
                  </button>
